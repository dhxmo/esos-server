...
describe('Admin Functionality', () => {
  ...
  describe('Admin service', () => {
    it('registers an admin in the database', async () => {
      ...
      const result = await adminServices.adminRegister(phoneNumber, password);
    ...
    it('returns an error if the phone number is not found', async () => {
      try {
        await adminServices.adminLogIn('0987654321', 'password123');
    ...
    it('returns an error if the password is incorrect', async () => {
      try {
        await adminServices.adminLogIn('1234567890', 'wrongpassword');
        expect.fail('Expected error to be thrown.');
      } catch (error) {
    ...
    it('should update to status banned of a user', async () => {
      let p = '0000000001';
      await adminServices.adminBanUserStatus(p, true);
    ...
    it('should update to status unbanned of a user', async () => {
      let p = '0000000001';
      await adminServices.adminBanUserStatus(p, false);
    ...
    it('should register a new ambulance driver and create a new driver live location entry', async () => {
      ...
      await adminServices.adminRegisterAmbulanceDriver(
        phoneNumber,
        password,
        companyName,
        ambulanceType
